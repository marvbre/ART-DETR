...
class Hiera(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        print("IMage after patch embed:", x.shape)
        x = x + self.get_pos_embed()
        print("IMage after pos embed:", x.shape)
        x = self.unroll(x)
        print("IMage after unroll:", x.shape)
...
@register()
class HybridEncoderReplacement(nn.Identity):
    def __init__(self):
        super().__init__()
    def forward(self, x):
        return x
